...
즉, 그변수의 범위는 함수 안에서만 유효하다.
이경우 그 함수의 지역(local, 로컬)을 변수의 스코프(범위,scope)라 한다.
함수내에서 선언된 변수를 지역변수(로컬변수)라고 한다.
'''
#[[지역변수의 선언예]]
a = 20        #전역변수 (global 변수), 여기서 선언된 변수a는 프로그램이 종료 될때까지 유효하다.

def func(a):   #()안에 a라는 매개변수(인수)는 로컬 변수, 즉 func함수에서 선언된 지역변수
	print('a는',a)
	a=10      #지역변수 a
	print("로컬 변수 a는",a)
...
bb = 100
def bb_func():
	global bb    # global함수식은 함수안에서 함수밖의 변수를 직접 사용하겠다라는 의미
	bb+=1
