 ''' 함수(function) : 재사용 가능한 프로그램, 명령 덩어리를 의미

	우리가 접했던 함수 중에 range(1,5) 함수는 
    이미 파이썬 안에 만들어져있는 함수(내장되어 있는) 내장함수였다. 
	앞에서 우리는 range()함수를 호출해서 사용했다.

	함수는 프로그램을 작성할 떄 필요한 중요한 단위이다.
	...
	[[문자열 함수]]
	  .문자열 포맷팅 함수 format()
'''
# {n}는 자리표시자를 의미한다. 여기서 n은 숫자를 의미한다. format()에 지정된 위치(인덱스)를 표시한다. 
...
#인덱스와 이름을 혼용해서 사용하기
print("오늘은 우리가 사귄지 {0}일 째 되는날!! 요일은 {day}" .format(122,day="thursday"))
...
#문자의 위치 찾기 함수: 문자열에서 찾고자하는 문자가 첫번째 위치를 리턴하는 함수

bb = "cafdefff"
loc =bb.find("h") # 찾고자 하는 문자가 없는 경우에는 -1을 반환한다.
print(loc)
'''
#오류, loc = bb.index("h") #index함수는 find함수와는 다르게 찾고자 하는 문자가 없을 경우 에러가난다.
print(loc)
'''
...
print(aa.strip()+"morning") #strip 양쪽 공백 모두지우기임

#문자열 대체 함수(replace) : 문자열 내에 특정한 값을 다른 값으로 교체한다.
aa="good morning jane!!!"
bb=aa.replace("morning","night")   # 바꿀대상(문자열), 교체할 대상(문자열)
print(bb)
...
str = "김진섭/23/서울시 강남구/010-3367-8853"
